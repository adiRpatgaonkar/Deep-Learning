...
def setup_hardware():
    ...
    if do.use_gpu: # Want GPU
        if (-1 < do.args.GPU_ID < torch.cuda.device_count()) and torch.cuda.is_available(): # GPU_ID exists & available
            # Subject to change
            torch.cuda.set_device(do.args.GPU_ID) 
            ...
            print("Selected GPU %d is NOT available." % do.args.GPU_ID)
            do.use_gpu = False
            print('\nUSING CPU.')
